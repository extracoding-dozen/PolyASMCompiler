// Package ir предоставляет функционал для генерации, трансформации и трансляции промежуточного представления кода.
package ir
...
// Generator отвечает за обход абстрактного синтаксического дерева (AST) и генерацию списка IR-инструкций.
// Поддерживает управление виртуальными регистрами, метками и таблицей символов переменных.
...
// NewGenerator создает новый экземпляр генератора IR с привязкой к общему контексту компиляции.
...
// emit добавляет новую инструкцию в общий поток выполнения.
func (g *Generator) emit(inst Instruction) {
	g.Instructions = append(g.Instructions, inst)
}

// Generate запускает процесс генерации IR-кода для всей программы.
...
// generateStatement выполняет диспетчеризацию типов операторов AST для их преобразования в IR.
...
// generateExpression преобразует выражения AST в цепочки IR-инструкций и возвращает итоговый виртуальный регистр.
...
// Print выводит текстовое представление сгенерированных IR-инструкций в стандартный поток вывода.
